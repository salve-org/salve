...
These can be imported as string constants (`AUTOCOMPLETE`, `REPLACEMENTS`, `HIGHLIGHT`, `EDITORCONFIG`, `DEFINITION`, `HIGHLIGHT_TREE_SITTER`) for usage in calling requests and requesting output from the server while getting "spelling checks" from your code editor so to speak (E.G. not very hard to misspell "autocomplete" as "autocopmlete").
...
### `make_unrefined_mapping(language: str, custom_highlights: list[Token], avoid_list: list[str]) -> dict[str, str]`

This function takes a language as a string, the default highlight output for the same input, and a list of token types to avoid (run this a few times adding the list until you remove all the undesired tokens (E.G. `if_statement`)). See the [simple mapping example](./examples/simple_mapping_example.py) for help.
...
| `.request()`        | (Tree Sitter Highlighting) Requests Tree Sitter highlighting instead of normal highlights (faster but requires more work)                                                                                                                                                                                                                                                                                                                              | `command`: str, `file`: str, `language`: str , `text_range`: tuple[int, int], `tree_sitter_language`: Callable (specifically the language() function in a tree sitter pypi release or the language_function() [here](https://github.com/tree-sitter/py-tree-sitter/discussions/251#discussioncomment-10015249)), `mapping`: dict[str, str] (see one of the examples as an example) |
